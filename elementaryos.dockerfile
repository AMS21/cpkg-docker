...
FROM ghcr.io/elementary/docker:$TAG
...
RUN set -eux; \
    ...
    apt update; \
    apt upgrade -y; \
    apt install ca-certificates curl gcc gcc-multilib git libssl-dev pkgconf -y --no-install-recommends; \
    ...
    apt remove curl libssl-dev pkgconf -y --auto-remove; \
    apt clean; \
